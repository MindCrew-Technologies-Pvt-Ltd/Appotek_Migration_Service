...
begin
	...
	if return_count > 0 
	then
		update sharing_tasks set accepted_by = applicant_id, accepted_date = current_timestamp, updated_at = current_timestamp where id = task_id;
	else
...
begin
	RETURN QUERY
		...
		select st.id, st.owner_id, st.created_at, st.accepted_date as accepted_at, st.description 
			...
			where ut.tagged_user_id = applicant_id and ps.tasks = true and (st.accepted_by is null or st.accepted_by = applicant_id) and st.deleted_at is null;
END;
$$
  LANGUAGE plpgsql; 

DROP FUNCTION sharing_get_users_for_task(integer);
