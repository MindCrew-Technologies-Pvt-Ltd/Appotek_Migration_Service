...
CREATE TYPE chat_message_type as enum ('text', 'file_attachment', 'location_attachment', 'contact_attachment', 'call_info');
ALTER TABLE chat_message
  ALTER COLUMN type TYPE chat_message_type using type::chat_message_type;

ALTER TABLE chat_message
  ADD COLUMN call_id UUID references calls (id) ON DELETE CASCADE ON UPDATE CASCADE;
