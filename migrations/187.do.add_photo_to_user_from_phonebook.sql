ALTER TABLE users_phone_book
	...
	ADD CONSTRAINT fk_user_phone_book_to_attachments FOREIGN KEY ("attachment_id") REFERENCES attachments(id) MATCH FULL ON UPDATE CASCADE;
...
begin
	return query
		...
		LEFT JOIN attachments a ON (u.attachment_id = a.id)
		left join guardians g1 on (g1.id = g.id and g1.first_user_approve = false)
		left join guardians g2 on (g2.id = g.id and g2.second_user_approve = false)
