ALTER TABLE users_phone_book
	ALTER COLUMN "telephoneId" DROP NOT NULL,
	ADD COLUMN "email" varchar NULL UNIQUE,
	ADD CONSTRAINT endpoint_check CHECK ((NOT (("telephoneId" IS NULL) AND ("email" IS NULL))));
...
CREATE TRIGGER after_email_created_trigger
  AFTER INSERT
  ON emails
  FOR EACH ROW
EXECUTE PROCEDURE after_email_created();
