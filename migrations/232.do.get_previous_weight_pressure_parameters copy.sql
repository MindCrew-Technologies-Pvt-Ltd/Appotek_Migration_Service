DROP FUNCTION public.user_pregnancy_get_summary_parameters(uuid, int);
CREATE OR REPLACE FUNCTION public.user_pregnancy_get_summary_parameters(_user_id uuid, pregnancy_id int default NULL)
  RETURNS TABLE
        (
			...
			blood_pressure_previous json,
			blood_pressure_last json,
			weight_first json,
			weight_previous json,
			weight_last json,
  ...
  DECLARE
	...
	_bloodPressure_previous json;
	_bloodPressure_last json;
	_weight_first json;
	_weight_previous json;
	_weight_last json;
...
begin
	...
	select json_build_object(
		'date', to_char(lp."date"::timestamptz, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'), 
