CREATE OR REPLACE FUNCTION delete_most_popular_licenses() RETURNS trigger AS $func$
    BEGIN
		IF (NEW.most_popular = true)
		THEN
			UPDATE license_tarif_plans SET most_popular = FALSE WHERE license_type = NEW.license_type AND id <> NEW.id;
		END IF;
	
		return new;
	
    END;
$func$ LANGUAGE plpgsql;

ALTER TABLE license_tarif_plans 
	RENAME COLUMN price TO actual_price;
UPDATE license_tarif_plans SET actual_price = actual_price + 1 WHERE actual_price IS NOT NULL;
UPDATE license_tarif_plans SET actual_price = -1 WHERE actual_price IS NULL;

CREATE TABLE license_tarif_plan_price_history (
	id serial NOT NULL PRIMARY KEY,
	tarif_plan_id int NOT NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	price decimal(12,2),
	CONSTRAINT tarif_plan_price_history_to_tarif_plans_fkey FOREIGN KEY (tarif_plan_id) REFERENCES license_tarif_plans(id) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE OR REPLACE FUNCTION license_set_actual_price() RETURNS trigger AS $func$
    BEGIN
		IF (TG_OP = 'INSERT' OR NEW.actual_price IS DISTINCT FROM OLD.actual_price)
		THEN
			IF (TG_TABLE_NAME = 'license_tarif_plan_to_user')
			THEN
				INSERT INTO license_custom_price_history (license_tarif_plan_to_user_id, price) VALUES (NEW.id, NEW.actual_price);
			END IF;

			IF (TG_TABLE_NAME = 'license_tarif_plans')
			THEN
				INSERT INTO license_tarif_plan_price_history (tarif_plan_id, price) VALUES (NEW.id, NEW.actual_price);
			END IF;
		END IF;
	
		return new;
    END;
$func$ LANGUAGE plpgsql;

CREATE TRIGGER license_set_actual_price AFTER INSERT OR UPDATE ON license_tarif_plans
    FOR EACH ROW EXECUTE PROCEDURE license_set_actual_price();
   
UPDATE license_tarif_plans SET actual_price = actual_price - 1 WHERE actual_price > 0;
UPDATE license_tarif_plans SET actual_price = NULL WHERE actual_price = -1;

ALTER TABLE license_tarif_plan_to_user 
	ADD COLUMN actual_price decimal(12,2);

CREATE TABLE license_custom_price_history (
	id serial NOT NULL PRIMARY KEY,
	license_tarif_plan_to_user_id int NOT NULL,
	created_at timestamptz NOT NULL DEFAULT now(),
	price decimal(12,2),
	CONSTRAINT custom_price_history_to_tarif_plan_to_user_fkey FOREIGN KEY (license_tarif_plan_to_user_id) REFERENCES license_tarif_plan_to_user(id) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TRIGGER license_set_actual_price AFTER INSERT OR UPDATE ON license_tarif_plan_to_user
    FOR EACH ROW EXECUTE PROCEDURE license_set_actual_price();
   
CREATE TABLE license_custom_property_values (
	id serial PRIMARY KEY,
	license_tarif_plan_to_user_id int NOT NULL,
	license_property_id int NOT NULL,
	value varchar NOT NULL,
	CONSTRAINT license_custom_property_ids_uniq UNIQUE (license_tarif_plan_to_user_id, license_property_id),
	CONSTRAINT license_custom_property_values_to_user_tarif_plan FOREIGN KEY (license_tarif_plan_to_user_id) REFERENCES license_tarif_plan_to_user(id) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE,	
	CONSTRAINT license_custom_property_values_to_properties FOREIGN KEY (license_property_id) REFERENCES license_properties(id) MATCH FULL ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE OR REPLACE FUNCTION check_license_types() RETURNS trigger AS $func$
DECLARE 
	_tarif_plan_type enum_license_type;
	_property_type enum_license_type;
BEGIN
	IF (TG_TABLE_NAME = 'license_property_values')
	THEN
		SELECT ltp.license_type INTO _tarif_plan_type FROM license_tarif_plans ltp WHERE ltp.id = NEW.license_tarif_plan_id;
	END IF;

	IF (TG_TABLE_NAME = 'license_custom_property_values')
	THEN
		SELECT ltp.license_type INTO _tarif_plan_type
		FROM license_tarif_plan_to_user ltptu 
		LEFT JOIN license_tarif_plans ltp ON (ltptu.license_tarif_plan_id = ltp.id)
		WHERE ltptu.id = NEW.license_tarif_plan_to_user_id;
	END IF;
	
	SELECT lp.license_type INTO _property_type FROM license_properties lp WHERE lp.id = NEW.license_property_id;

	IF (_tarif_plan_type <> _property_type) THEN 
		RETURN NULL;
	END IF;
	RETURN NEW;
END;
$func$ LANGUAGE plpgsql;

CREATE TRIGGER check_license_types BEFORE INSERT OR UPDATE ON license_custom_property_values
    FOR EACH ROW EXECUTE PROCEDURE check_license_types();